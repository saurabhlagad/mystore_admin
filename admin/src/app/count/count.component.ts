...
getFeedbackCar(){
  this.service.getFeedbackCount()
                .subscribe(response=>{
                  if(response['status']=='success')
                  {
                    console.log(`response data:${response['data']}`)
                    this.total_feedback_count=response['data'][0]['Feedback_count']
                  }
                  else{
                    this.toastr.error(response['error'])
                  }
...
getFAQCar(){
  this.service.getFAQCount()
                .subscribe(response=>{
                  if(response['status']=='success')
                  {
                    console.log(`response data:${response['data']}`)
                    this.total_faq_count=response['data'][0]['FAQ_count']
                  }
                  else{
                    this.toastr.error(response['error'])
                  }
